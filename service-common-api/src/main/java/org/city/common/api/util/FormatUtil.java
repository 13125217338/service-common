...
public final class FormatUtil {
	...
	private final static Map<Class<?>, Map<Field, Entry<Integer, FormatFieldValue>>> FIELD_CACHE = new ConcurrentHashMap<>();
	...
	public static void format(Object param) {
		...
		/* 获取所有缓存字段解析 */
		Map<Field, Entry<Integer, FormatFieldValue>> fields = FIELD_CACHE.computeIfAbsent(param.getClass(), pcls -> {
			Map<Field, Entry<Integer, FormatFieldValue>> cache = new HashMap<>();
			/* 获取所有非常量字段 */
			for (Field field : FieldUtil.getAllDeclaredField(pcls, false).values()) {
				...
				if (format.format() == FormatFieldValue.class) {cache.put(field, null);}
					...
					/* 字段解析 */
					cache.put(field, new Entry<Integer, FormatFieldValue>() {
						private final Integer type = format.type();
						...
						@Override
						public Integer getKey() {return this.type;}
					});
				}
			}
			return cache;
		...
		/* 处理待解析字段 */
		for (Entry<Field, Entry<Integer, FormatFieldValue>> entry : fields.entrySet()) {
			if (entry.getValue() == null) { //嵌套解析
				try {format(entry.getKey().get(param));}
				catch (Exception e) {throw new RuntimeException("自定义格式化特定类型错误！", e);}
			} else { //字段解析
				Entry<Integer, FormatFieldValue> ey = entry.getValue();
				try {entry.getKey().set(param, ey.getValue().format(entry.getKey(), entry.getKey().get(param), ey.getKey().intValue()));}
