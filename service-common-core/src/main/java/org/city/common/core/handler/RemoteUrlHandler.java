...
public class RemoteUrlHandler implements InvocationHandler,JSONParser,Replace,Validations {
	private final RestTemplate restTemplate = new RestTemplate(SpringUtil.getBean(MvcConfig.class).setConverter(new ArrayList<>(), null));
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		return SpeedLimitUtil.limitExec(remoteUrl.speedLimit(), method, () -> new RemoteMethodSpeedLimitException(url, method), () -> invoke(method, args == null ? new Object[0] : args));
	}
	/* 执行远程调用 */
	private Object invoke(Method method, Object[] args) {
		...
		if (mapping.containsKey(PostMapping.class.getName())) { //POST调用
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.POST, getHttpEntity(reqPath, method, uriVariables, postMapping.headers(), args), String.class, uriVariables);
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.GET, getHttpEntity(reqPath, method, uriVariables, getMapping.headers(), args), String.class, uriVariables);
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.DELETE, getHttpEntity(reqPath, method, uriVariables, deleteMapping.headers(), args), String.class, uriVariables);
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.PUT, getHttpEntity(reqPath, method, uriVariables, putMapping.headers(), args), String.class, uriVariables);
	...
	/* 获取请求实体类 */
	private HttpEntity<Object> getHttpEntity(String url, Method method, Map<String, Object> uriVariables, String[] headers, Object[] args) {
		...
		if (remoteUrl.requestBody() == RemoteUrlRequestBody.class) {
			...
			body = getRemoteUrlRequestBody().getBody(url, method, header, args, uriVariables);
		}
