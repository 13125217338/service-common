 ...
 * @描述 替换特殊字符接口
 */
public interface Replace extends AuthIsJsonParse,ThrowableMessage {
	/**
	 ...
	 * @throws Exception 
	 */
	default void replaceVals(Environment environment, String[] values, Object curObj, Object[] datas, String[] names) throws Exception {
	 ...
	 * @throws Exception
	 */
	default String replaceMethod(Object curObj, String val, Object[] args) throws Exception {
		...
		for (Entry<String, String> cv : MyUtil.getContainValue("&{", "}", val).entrySet()) {
			...
			if (method == null) {throw new RuntimeException(String.format("当前类[%s]执行方法[%s]不存在！", curObjClass.getName(), cv.getValue()));}
			
			/* 设置权限执行方法 */
			method.setAccessible(true);
			/* 执行结果 */
			Object property = method.invoke(curObj, args);
