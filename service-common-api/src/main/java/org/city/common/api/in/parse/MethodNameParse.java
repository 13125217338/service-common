...
public interface MethodNameParse {
	...
	default AuthMethod toAuthMethod(Method method, org.city.common.api.annotation.auth.Auth auth, Auths auths) {
		...
		/* 解析验证注解 */
		if (auth == null) {
			for (org.city.common.api.annotation.auth.Auth at : auths.auths()) {
		...
		return new AuthMethod(method.getDeclaringClass().getName(), method.getName(), parameterType, method.getGenericReturnType().getTypeName(), ats, null);
	}
}
