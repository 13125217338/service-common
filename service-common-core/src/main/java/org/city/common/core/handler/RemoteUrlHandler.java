 ...
 */
@Slf4j
public class RemoteUrlHandler implements InvocationHandler,JSONParser,Replace {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		SpeedLimitUtil.limitSec(remoteUrl.speedLimit(), 1, method, () -> new RemoteSpeedLimitException("远程服务限流！"));
		...
		if (mapping.containsKey(PostMapping.class.getName())) { //POST调用
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.POST, getHttpEntity(method, postMapping.headers(), args), String.class, uriVariables);
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.GET, getHttpEntity(method, getMapping.headers(), args), String.class, uriVariables);
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.DELETE, getHttpEntity(method, deleteMapping.headers(), args), String.class, uriVariables);
			...
			ResponseEntity<String> response = restTemplate.exchange(reqPath, HttpMethod.PUT, getHttpEntity(method, putMapping.headers(), args), String.class, uriVariables);
	...
	private Object getResult(Method method, HttpHeaders responseHeaders, String body) {
		...
		else {return SpringUtil.getBean(remoteUrl.responseBody()).handler(method, responseHeaders, body);}
	}
	
	/* 获取请求实体类 */
	private HttpEntity<Object> getHttpEntity(Method method, String[] headers, Object[] args) {
		...
		if (remoteUrl.commonHeaders() != RemoteUrlHeaders.class) {
			header.addAll(SpringUtil.getBean(remoteUrl.commonHeaders()).getHeaders(method));
		}
		...
		if (remoteUrl.requestBody() == RemoteUrlRequestBody.class) {
			...
			body = SpringUtil.getBean(remoteUrl.requestBody()).getBody(method, header, args);
		}
