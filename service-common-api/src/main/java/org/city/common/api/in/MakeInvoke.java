...
public interface MakeInvoke {
	...
	public interface Process {
		...
		/**
		 * @描述 获取执行过的对象
		 * @return key=执行对象，value=执行参数
		 */
		public LinkedHashMap<org.city.common.api.in.MakeInvoke, MakeInvokeDto> getMakes();
		/**
		 * @描述 获取所有执行的对象（不管执没执行过）
		 * @return key=执行对象，value=执行参数
		 */
		public Map<org.city.common.api.in.MakeInvoke, MakeInvokeDto> getAllMakes();
		/**
		 * @描述 设置自定义参数
		 * @param key 键
		 * @param value 值
