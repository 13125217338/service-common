 ...
 * @描述 注解条件服务
 */
class AnnotationConditionService<D extends BaseDto> implements AnnotationCondition<D>,JSONParser {
	private final Condition condition;
	private final Crud<? extends BaseDto> curService;
	private AnnotationConditionService(Condition condition, Crud<? extends BaseDto> curService) {
		this.condition = condition; this.curService = curService;
	}
	
	/* 生成注解条件服务 */
	static <D extends BaseDto> AnnotationCondition<D> getJoin(Class<?> DTO_CLASS, Crud<? extends BaseDto> curService, int... groups) {
		if (groups == null) {throw new NullPointerException("分组条件可以不传，但是不能为空！");}
		
		/* 生成条件 */
		Condition condition = new Condition();
		AnnotationConditionService<D> conditionService = new AnnotationConditionService<D>(condition, curService);
	...
	public AnnotationCondition<D> getNotEmpty(BaseDto d, int... groups) {
		...
		if (groups == null) {throw new NullPointerException("分组条件可以不传，但是不能为空！");}
		List<DataCondition<Field, org.city.common.api.annotation.sql.Condition>> fcs = getCondition(curService.getDtoFields(), groups); //获取所有条件
		...
		try {
			...
			for (DataCondition<Field, org.city.common.api.annotation.sql.Condition> fc : fcs) {
				...
				Object val = getFieldValue(d, field, cd); //取出对象字段值
				
				if (isBaseType(field.getType())) { //基本类型处理 - 当前对象
					setBaseCondition(field, cd, val, null);
				} else {
					if (cd.joinTable().join() != Crud.class) {
						Crud<?> crud = condition.getCrud(cd.joinTable().join());
						if (val instanceof Collection && ((Collection<?>) val).size() > 0) {val = ((Collection<?>) val).iterator().next();} //如果字段值是集合则取第一个值
						if (val instanceof BaseDto && crud != null) { //添加连接条件
							List<DataCondition<Field, org.city.common.api.annotation.sql.Condition>> fcsJoin = getCondition(crud.getDtoFields(), groups);
							...
							for (DataCondition<Field, org.city.common.api.annotation.sql.Condition> fcJoin : fcsJoin) {
								...
								Object valJoin = getFieldValue((BaseDto) val, fieldJoin, cdJoin); //取出连接对象字段值
								if (isBaseType(fieldJoin.getType())) { //基本类型处理 - 连接对象
	...
	/* 获取所有条件 */
	private List<DataCondition<Field, org.city.common.api.annotation.sql.Condition>> getCondition(Map<String, Field> dtoFields, int... groups) {
		/* 只取有条件的字段 */
		List<DataCondition<Field, org.city.common.api.annotation.sql.Condition>> fcs = new ArrayList<>();
		for (Field field : dtoFields.values()) {
	...
	private Object getFieldValue(BaseDto d, Field field, org.city.common.api.annotation.sql.Condition cd)
		...
		if (StringUtils.hasText((String) val)) {
			...
			if (cd.alias().length() == 0) {val = field.get(d);}
				...
				Field aliasField = curService.getDtoFields().get(cd.alias());
	...
	private void setBaseCondition(Field field, org.city.common.api.annotation.sql.Condition cd, Object val, JoinTable joinTable) {
		/* 特殊条件处理 - 否则没有值直接跳过 */
		if (Operation.Is_Null != cd.make() && Operation.Is_Not_Null != cd.make()) {
			if (val == null || !StringUtils.hasText(val.toString())) {return;}
		}
	...
	public AnnotationCondition<D> getField(int... groups) {
		...
		for (Field field : curService.getDtoFields().values()) {
		...
		for (DataCondition<Field, org.city.common.api.annotation.sql.Field> fc : fcs) {
			if (fc.annotation.joinTable().join() == Crud.class) {
				...
				Crud<?> joinService = condition.getCrud(fc.annotation.joinTable().join());
				if (joinService != null) {
					/* 设置字段参数 */
					JoinTable joinTable = new JoinTable(joinService, fc.annotation.joinTable().alias());
					if (isBaseType(fc.data.getType())) {
						join.setJoinTable(joinTable).addField(fc.annotation.mathSql(), fc.data.getName(), fc.annotation.joinTable().joinFieldName(), getVals(fc.annotation, joinService));
					/* 非基本类型使用子对象模式装填 */
					} else {join.setJoinTable(joinTable).addField(fc.data.getName());}
				}
			}
		}
		return this;
	...
	/* 获取自定义函数值 */
	private String[] getVals(org.city.common.api.annotation.sql.Field field, Crud<? extends BaseDto> join) {
		if (field.mathSqlVal() == MathSqlValue.class) {return field.vals();}
		else {return SpringUtil.getBean(field.mathSqlVal()).getVals(curService, join, field.vals());}
	}
