 ...
 * @描述 公共的条件
 */
public class BaseVo<D extends BaseEntity> extends BaseEntity {
	 ...
	 * @描述 结果
	 */
	public static class Result<D extends BaseEntity> implements JSONParser {
		...
		private Object parseVo(Field field, Object dto) throws Exception {
			if (Result.class.isAssignableFrom(field.getType())) {
				return ((Result<D>) field.getType().getConstructor().newInstance()).from(dto);
			} else if (Collection.class.isAssignableFrom(field.getType())) {
				Class<?> fieldType = (Class<?>) ((ParameterizedType) field.getGenericType()).getActualTypeArguments()[0];
				if (Result.class.isAssignableFrom(fieldType)) {return ((Result<D>) fieldType.getConstructor().newInstance()).from(dto);}
