...
public class RemoteHandler implements InvocationHandler,MethodNameParse,JSONParser,Validations {
	...
	public RemoteHandler(RemoteInfo remoteInfo, RemoteInvokeApi remoteInvokeApi, int failTime) {
		this.remoteInfo = remoteInfo; this.remoteInvokeApi = remoteInvokeApi; this.failTime = failTime;
	}
	...
	private Object invoke(Method method, Object[] args) throws Throwable {
		...
		return exec(method, args, remoteInfo.getRemoteClassDto().getBeanName(), remoteInfo.getSpeedLimit(), remoteMethodDto); //执行远程调用
	}
	/* 正常执行 */
	private Object exec(Method method, Object[] args, String beanName, Integer speedLimit, RemoteMethodDto remoteMethodDto) throws Throwable {
		RemoteAdapter remoteAdapter = remoteInfo.getRemoteAdapter(); //当前远程适配器
		long recordTime = System.currentTimeMillis(); //记录当前时间
		try {return remoteInvokeApi.invoke(beanName, speedLimit, remoteMethodDto, remoteInfo.getRemoteIpPortDto(), args);} //远程调用
		/* 连接异常不可用标记 */
		catch (Throwable e) {if (CommonConstant.isConnectTimeout(e) || CommonConstant.isConnectTimeout(e.getCause())) {remoteInfo.setTurnOnTime(System.currentTimeMillis() + failTime);} throw e;}
		/* 回调执行时间与远程信息 */
