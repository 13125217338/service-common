...
public final class SpeedLimitUtil {
	...
	public static <T> T limitExec(int sum, Object syncObj, Supplier<RuntimeException> speedEx, FunctionResponse<T> passExec) throws Throwable {
		...
		try {
			...
			if (isAdd) {
				/* 只能一个对象执行 */
				synchronized (syncObj) {
					Integer curSum = LIMIT_EXEC.get(syncObj) - 1; //请求数量减1
					LIMIT_EXEC.put(syncObj, curSum); //更新请求数量
					syncObj.notifyAll();
				}
				
				/* 全量清理过期条目（在锁内，线程安全） */
				LIMIT_EXEC.entrySet().removeIf(entry -> {
	 ...
	 * @param speedEx 超速异常（NULL=同步限制）
	 */
	public static void limitSec(int sum, long timeSec, Object syncObj, Supplier<RuntimeException> speedEx) {
		if (sum < 1 || timeSec < 1) {return;} else {timeSec = timeSec * 1000;} //小于1不限制，时间换成毫秒计算增加精准度
		/* 只能一个对象执行 */
		synchronized (syncObj) {
			final long timeSecMillis = timeSec;
			LimitInfo limitInfo = LIMIT_SEC.computeIfAbsent(syncObj, k -> new LimitInfo(timeSecMillis, 0, System.currentTimeMillis())); //限制信息
		...
		LIMIT_SEC.entrySet().removeIf(entry -> {
			synchronized (entry.getKey()) {
				return (now - entry.getValue().recordTime) > entry.getValue().timeSecMillis;
			}
		});
	}
	/* 限制减少计算 */
