...
public final class StreamUtil {
	 ...
	 */
	public static void readNBytes(InputStream in, long len, Consumer<Entry<byte[], Integer>> readCall) {
		try {
			/* 读取详情 */
			...
			while (len > 0) {
				...
				if (read == -1) {throw new ClosedChannelException();} //未读取到指定长度数据 - 如果连接关闭则抛出异常
