...
public class TransactionalSubAop {
	...
	private Object handler(String tranId, Method method, ProceedingJoinPoint jp) throws Throwable {
		boolean isSendTran = transactionalAop.TRAN_DATA.get(tranId) == null;
		if (isSendTran) {remoteTransactional.add(tranId);} else {remoteTransactional.append(tranId);} //发起者添加事务 - 参与者追加事务
		...
		if (!isSendTran) {synchronized (tranId) {tranId.notifyAll();}} //参与者通知主线程运行返回值
