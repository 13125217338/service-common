...
public final class PlugUtil implements BeanPostProcessor{
	private static RemoteSaveAdapter remoteSaveAdapter;
	@Autowired
	private void init(GlobalConfig globalConfig, ApplicationContext applicationContext) throws Exception {
		...
		PlugUtil.remoteSaveAdapter = globalConfig.init(applicationContext);
		PlugUtil.remoteSaveAdapter.init(); //初始化远程保存信息
	}
	 ...
	 * @return 远程信息
	 */
	public static <T> RemoteInfo getRemote(Object param, RemoteAdapter remoteAdapter, Class<T> interfaceCls) {
		RemoteInfo select = remoteAdapter.select(remoteSaveAdapter.get(interfaceCls), param);
	 ...
	 * @return 所有远程信息
	 */
	public static <T> List<RemoteInfo> getRemotes(Class<T> interfaceCls) {
		return remoteSaveAdapter.get(interfaceCls);
	}
	...
	public static MethodProcess getMethod(Object param, RemoteAdapter remoteAdapter, String methodName) {
		RemoteInfo select = remoteAdapter.select(remoteSaveAdapter.get(methodName), param);
		Assert.notNull(select, String.format("方法[%s]使用参数[%s]的选择器[%s]返回NULL未选择任何实现类！", methodName, param, remoteAdapter.getClass().getName()));
		/* 取出相应参数 */
		RemoteMethodDto remoteMethodDto = select.getRemoteClassDto().getMethods().values().stream()
	...
	public static class MethodProcess implements JSONParser,MethodNameParse{
		...
		public final Object invoke(Object...args){
			...
			for (int i = 0, j = args.length; i < j; i++) {args[i] = parse(args[i], types[i]);}
			/* 执行方法 */
