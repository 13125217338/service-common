...
public final class RedisTokenMake implements JSONParser {
	...
	private final RedisMake REDIS_MAKE;
	private final Task TASK;
	private final RemoteConfigDto REMOTE_CONFIG_DTO;
	private final List<RedisTokenRemove> REDIS_TOKEN_REMOVES;
	private final String ONLY_ID;
	/**
	 * @param onlyId 识别验证数据唯一ID
	 * @param redisMake 缓存操作
	 */
	public RedisTokenMake(String onlyId, RedisMake redisMake) {
		ONLY_ID = onlyId;
		REDIS_MAKE = redisMake;
		TASK = SpringUtil.getBean(Task.class);
		REMOTE_CONFIG_DTO = SpringUtil.getBean(RemoteConfigDto.class);
		REDIS_TOKEN_REMOVES = SpringUtil.getBeans(RedisTokenRemove.class);
		init(); //初始化
	}
	/* 初始化 */
	private void init() {
		TASK.schedula(CommonConstant.REDIS_TOKEN_TIME_EXPIRE_ID + SPLIT + ONLY_ID, new Runnable() {
			...
			public void run() throws Throwable {
				Long size = REDIS_MAKE.getRedisTemplate().opsForHash().size(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID);
				new BaseEntity().setPageSize((long) Short.MAX_VALUE).limitHandler(size.longValue(), new Limit() { //分页验证所有数据
					...
					public void handler(BaseEntity baseEntity) {
						DataList<Entry<Object, Object>> entrys = REDIS_MAKE.entry(RedisDto.of(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, "*", baseEntity.getPageNum(), baseEntity.getPageSize()));
						handler(entrys.getRows(), REDIS_MAKE.getNowTime());
					}
					/* 处理分页数据 */
					private void handler(List<Entry<Object, Object>> rows, long nowTime) {
						for (Entry<Object, Object> entry : rows) {
							...
							REDIS_MAKE.tryLock(CommonConstant.REDIS_TOKEN_HKEY + SPLIT + ONLY_ID + tkId, REMOTE_CONFIG_DTO.getReadTimeout(), true, () -> {
		...
		}, () -> String.valueOf(REMOTE_CONFIG_DTO.getTokenExpireTime() >> 2), false);
	}
	...
	public Map<String, Object> getAll() {
		return REDIS_MAKE.entry(CommonConstant.REDIS_TOKEN_HKEY + SPLIT_DATA + ONLY_ID);
	}
	...
	public String getToken(String onlyId, Object data) {
		/* 生成令牌 */
		long nowTime = REDIS_MAKE.getNowTime();
		String token = MyUtil.md5(JSON.toJSONString(data) + nowTime);
		/* 删除重复的其他令牌数据 */
		deleteByToken(token); deleteByOnlyId(onlyId);
		
		/* 记录过期时间与令牌数据 */
		REDIS_MAKE.setHValue(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, token + SPLIT + onlyId, nowTime);
		REDIS_MAKE.setHValue(CommonConstant.REDIS_TOKEN_HKEY + SPLIT_DATA + ONLY_ID, token + SPLIT + onlyId, data);
		return token;
	}
	...
	public String getTkIdByToken(String token) {
		DataList<Entry<Object, Object>> entrys = REDIS_MAKE.entry(RedisDto.of(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, token + SPLIT + "*", 1, Short.MAX_VALUE));
		return getTkId(entrys.getRows(), REDIS_MAKE.getNowTime(), true).stream().findFirst().orElse(null);
	}
	/**
	 * @描述 获取令牌与唯一ID来自唯一ID
	 * @param onlyId 唯一ID
	...
	public String getTkIdByOnlyId(String onlyId) {
		DataList<Entry<Object, Object>> entrys = REDIS_MAKE.entry(RedisDto.of(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, "*" + SPLIT + onlyId, 1, Short.MAX_VALUE));
		return getTkId(entrys.getRows(), REDIS_MAKE.getNowTime(), true).stream().findFirst().orElse(null);
	}
	/**
	...
	public List<String> getTkIdsByOnlyId(String onlyId) {
		DataList<Entry<Object, Object>> entrys = REDIS_MAKE.entry(RedisDto.of(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, "*" + SPLIT + onlyId + "*", 1, Short.MAX_VALUE));
		return getTkId(entrys.getRows(), REDIS_MAKE.getNowTime(), false);
	}
	/* 获取令牌与唯一ID */
	private List<String> getTkId(List<Entry<Object, Object>> rows, long nowTime, boolean hasOne) {
		...
		for (Entry<Object, Object> entry : rows) {
			final String tkId = entry.getKey().toString(); //令牌ID - 用于加锁验证过期时间
			final String curTkId = REDIS_MAKE.tryLock(CommonConstant.REDIS_TOKEN_HKEY + SPLIT + ONLY_ID + tkId, REMOTE_CONFIG_DTO.getReadTimeout(), true, () -> {
				Long tkTime = REDIS_MAKE.getHValue(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, tkId, Long.class);
				return verifyExpire(tkId, tkTime, nowTime);
			});
			/* 有令牌ID则添加 - 否则继续查询 */
			if (curTkId != null) {
	...
	public <T> T getByTkId(String tkId, Type type) {
		...
		return REDIS_MAKE.getHValue(CommonConstant.REDIS_TOKEN_HKEY + SPLIT_DATA + ONLY_ID, tkId, type);
	}
	...
	public long getRunTimeByTkId(String tkId) {
		...
		long recordTime = REDIS_MAKE.getHValue(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, tkId, long.class);
		return REDIS_MAKE.getNowTime() - recordTime;
	}
	...
	public void flushByTkId(String tkId) {
		if (tkId != null) {REDIS_MAKE.setHValue(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, tkId, REDIS_MAKE.getNowTime());}
	}
	...
	public void setByTkId(String tkId, Object data) {
		if (tkId != null) {REDIS_MAKE.setHValue(CommonConstant.REDIS_TOKEN_HKEY + SPLIT_DATA + ONLY_ID, tkId, data);}
	}
	...
	private String verifyExpire(String tkId, Long recordTime, long nowTime) {
		...
		boolean expire = (nowTime - recordTime) > REMOTE_CONFIG_DTO.getTokenExpireTime(); //是否过期
		if (expire) {deleteByTkId(tkId); return null;} else {return tkId;}
	}
	/* 移除令牌数据与过期时间 */
	private void $delete(String removeTkId) {
		if (!CollectionUtils.isEmpty(REDIS_TOKEN_REMOVES)) { //回调自定义移除通知接口
			for (RedisTokenRemove redisTokenRemove : REDIS_TOKEN_REMOVES) {
				redisTokenRemove.remove(removeTkId, getByTkId(removeTkId, Object.class));
			}
		}
		/* 删除缓存 */
		REDIS_MAKE.delHKey(CommonConstant.REDIS_TOKEN_HKEY + SPLIT_DATA + ONLY_ID, Arrays.asList(removeTkId));
		REDIS_MAKE.delHKey(CommonConstant.REDIS_TOKEN_EXPIRE_TIME_HKEY + SPLIT_DATA + ONLY_ID, Arrays.asList(removeTkId));
	}
}
