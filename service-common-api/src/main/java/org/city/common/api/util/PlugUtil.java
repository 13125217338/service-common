...
public final class PlugUtil {
	 ...
	 * @return 所有远程信息
	 */
	public static List<RemoteInfo> getRemotes(String methodName) {
		return REMOTE_SAVE.get(methodName);
	}
	/**
	 * @描述 分组获取所有远程信息（集群服务合并为一个服务）
	 * @param interfaceCls 对象的接口
	 * @return 分组后的所有远程信息
	 */
	public static List<RemoteInfo> getRemotesByGroup(Class<?> interfaceCls) {
		return getRemotes(interfaceCls).stream().collect(Collectors.groupingBy(v -> v.getRemoteClassDto().getName())).values()
				.stream().map(v -> v.iterator().next()).collect(Collectors.toList());
	}
	/**
	 * @描述 分组获取所有远程信息（集群服务合并为一个服务）
	 * @param methodName 方法名称
	 * @return 分组后的所有远程信息
	 */
	public static List<RemoteInfo> getRemotesByGroup(String methodName) {
		return getRemotes(methodName).stream().collect(Collectors.groupingBy(v -> v.getRemoteClassDto().getName())).values()
				.stream().map(v -> v.iterator().next()).collect(Collectors.toList());
	}
	...
	/**
	 * @描述 分组获取所有接口对象（集群服务合并为一个服务）
	 * @param <T> 接口对象
	 * @param interfaceCls 对象的接口
	 * @return 分组后的所有接口对象
	 */
	@SuppressWarnings("unchecked")
	public static <T> List<T> getBeansByGroup(Class<T> interfaceCls) {
		return getRemotesByGroup(interfaceCls).stream().map(v -> (T) v.getBean()).collect(Collectors.toList());
	}
	...
	/**
	 * @描述 分组获取所有远程类信息（集群服务合并为一个服务）
	 * @param interfaceCls 对象的接口
	 * @return 分组后的所有远程类信息
	 */
	public static List<RemoteClassDto> getValuesByGroup(Class<?> interfaceCls) {
		return getRemotesByGroup(interfaceCls).stream().map(v -> v.getRemoteClassDto()).collect(Collectors.toList());
	}
	...
	public static MethodProcess getMethod(Object param, RemoteAdapter remoteAdapter, String methodName) {
		...
		if (select == null) {
			log.error("方法[{}]使用参数[{}]的选择器[{}]未选择任何实现类！", methodName, param, remoteAdapter.getClass().getName());
			throw new ServiceNotFoundException(param, remoteAdapter, methodName);
		}
		return getMethodProcess(select, methodName);
	}
	/**
	 * @描述 获取所有方法执行器
	 * @param methodName 方法名称
	 * @return 所有方法执行器
	 */
	public static List<MethodProcess> getMethods(String methodName) {
		return getRemotes(methodName).stream().map(v -> getMethodProcess(v, methodName)).collect(Collectors.toList());
	}
	/**
	 * @描述 分组获取所有方法执行器（集群服务合并为一个服务）
	 * @param methodName 方法名称
	 * @return 分组后的所有方法执行器
	 */
	public static List<MethodProcess> getMethodsByGroup(String methodName) {
		return getRemotesByGroup(methodName).stream().map(v -> getMethodProcess(v, methodName)).collect(Collectors.toList());
	}
	/* 获取方法执行器 */
	private static MethodProcess getMethodProcess(RemoteInfo remoteInfo, String methodName) {
		RemoteClassDto remoteClassDto = remoteInfo.getRemoteClassDto();
		RemoteMethodDto remoteMethodDto = remoteClassDto.getMethods().values().stream().filter(v -> {
			RemoteMethod remoteMethod = v.getAnnotation(RemoteMethod.class);
			return remoteMethod == null ? false : (remoteMethod.name().equals(methodName));
		}).findFirst().orElseThrow(() -> new IllegalArgumentException(String.format("远程类[%s]中未找到方法[%s]！", remoteClassDto.getName(), methodName)));
		return new MethodProcess(remoteInfo.getBean(), remoteClassDto.getInterfaceName(), remoteClassDto.getName(), remoteMethodDto);
	}
