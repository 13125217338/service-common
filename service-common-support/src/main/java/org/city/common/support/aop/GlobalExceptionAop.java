...
public class GlobalExceptionAop implements JSONParser,ThrowableMessage,GlobalExceptionParse {
	@Autowired(required = false)
	private List<ExceptionParse<? extends Throwable>> exceptionParses;
	
	@PostConstruct
	private void init() {
		if (!CollectionUtils.isEmpty(exceptionParses)) { //自定义异常排序
			exceptionParses.sort(new Comparator<ExceptionParse<? extends Throwable>>() {
				...
				public int compare(ExceptionParse<? extends Throwable> o1, ExceptionParse<? extends Throwable> o2) {
					...
					return t1.isAssignableFrom(t2) ? 1 : (t2.isAssignableFrom(t1) ? -1 : 0); //范围较大的异常放后面，先处理小异常
				}
			});
		}
	}
	...
	public Object exceptionHandler(Throwable e) {
		...
		if (response != null) { //翻译异常信息
			response.setMsg(LanguageUtil.translation(null, response.getMsg(), response.getMsg()));
		}
		return response;
	}
	...
	public Response<?> getResponse(Throwable e) {
		...
		if (e instanceof ResponseException) {
			...
			String appName = responseExp.getAppName();
			RemoteIpPortDto appAddress = responseExp.getRemoteIpPort();
			Response<?> response = responseExp.getResponse();
			
			/* 打印并返回 */
			log.error("服务端[{}][{}]远程调用执行异常！ \r\n\tat {}", appName, appAddress, JSON.toJSONString(response.getData()));
			return response;
		}
