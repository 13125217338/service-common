package org.city.common.clickhouse.service;
...
class AnnotationConditionService<D extends BaseDto> implements AnnotationCondition<D>,JSONParser {
	...
	public AnnotationCondition<D> getNotEmpty(BaseDto d, int...groups) {
		...
		try {
			...
			for (DataCondition<Field, org.city.common.api.annotation.sql.Condition> fc : fcs) {
				...
				Object val = getFieldValue(d, field, cd, curService); //取出对象字段值
				
				if (isBaseType(field.getType()) || Date.class.isAssignableFrom(field.getType())) { //基本类型或时间类型处理 - 当前对象
					setBaseCondition(field, cd, val, null);
				} else {
					if (val != null && cd.joinTable().join() != Crud.class) {
						...
						if (val instanceof BaseDto && crud != null) { //添加连接条件
							List<DataCondition<Field, org.city.common.api.annotation.sql.Condition>> fcsJoin = getCondition(crud.getDtoFields(), cd.joinTable().groups());
							...
							for (DataCondition<Field, org.city.common.api.annotation.sql.Condition> fcJoin : fcsJoin) {
								...
								Object valJoin = getFieldValue((BaseDto) val, fieldJoin, cdJoin, crud); //取出连接对象字段值
	...
	private Object getFieldValue(BaseDto d, Field field, org.city.common.api.annotation.sql.Condition cd, Crud<?> service)
		...
		if (StringUtils.hasText((String) val)) {
			...
			if (cd.alias().length() == 0) {val = field.get(d);}
				...
				Field aliasField = service.getDtoFields().get(cd.alias());
