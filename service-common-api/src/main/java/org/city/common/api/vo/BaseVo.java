 ...
 * @描述 公共的条件
 */
public class BaseVo<T extends BaseDto> extends BaseDto {
	...
	private Class<T> dtoClass() {
		...
		while (curCls != Object.class) {
			if (curCls.getSuperclass() == BaseVo.class) {
				Type type = ((ParameterizedType) curCls.getGenericSuperclass()).getActualTypeArguments()[0];
				return type instanceof Class ? (Class<T>) type : (Class<T>) ((ParameterizedType) type).getRawType(); 
			} else {curCls = curCls.getSuperclass();}
		}
		throw new NullPointerException(String.format("当前条件类[%s]未继承[BaseVo]公共条件！", this.getClass().getName()));
	}
	/* 验证分组 */
	...
	private Object parseDto(Object vo, int...groups) throws Exception {
		if (vo instanceof BaseVo) {return ((BaseVo<?>) vo).to(groups);}
		else if (vo instanceof Collection) {
			JSONArray dtos = new JSONArray();
			for (Object v : (Collection<Object>) vo) {
				if (v != null && v instanceof BaseVo) {dtos.add(((BaseVo<?>) v).to(groups));}
				else {dtos.add(v);} //添加原值
			}
			return dtos;
		} else if (vo.getClass().isArray()) {
			int len = Array.getLength(vo);
			Object dtos = Array.newInstance(BaseDto.class, len);
			for (int i = 0; i < len; i++) {
				Object v = Array.get(vo, i);
				if (v != null && v instanceof BaseVo) {Array.set(dtos, i, ((BaseVo<?>) v).to(groups));}
	 ...
	 * @描述 结果
	 */
	public static class Result<T extends BaseDto> implements JSONParser {
		...
		private Object parseVo(Field field, Object dto) throws Exception {
			if (Result.class.isAssignableFrom(field.getType())) {
				return ((Result<T>) field.getType().getConstructor().newInstance()).from(dto);
			} else if (Collection.class.isAssignableFrom(field.getType())) {
				Class<?> fieldType = (Class<?>) ((ParameterizedType) field.getGenericType()).getActualTypeArguments()[0];
				if (Result.class.isAssignableFrom(fieldType)) {return ((Result<T>) fieldType.getConstructor().newInstance()).from(dto);}
