...
public interface Replace extends AuthIsJsonParse {
	...
	default void replaceVals(Environment environment, String[] values, Object curObj, Object[] datas, String[] names) {
		...
		for (int i = 0, j = values.length; i < j; i++) {
			...
			/* 替换执行方法 */
			values[i] = replaceMethod(curObj, values[i]);
		}
	}
	...
	default String replaceConfig(Environment environment, String val) {
		for (Entry<String, String> idEntry :  MyUtil.getContainValue("${", "}", val).entrySet()) {
			int mh = idEntry.getValue().indexOf(":"); 
			
			/* 查找配置Key */
			String eKey = mh == -1 ? idEntry.getValue() : idEntry.getValue().substring(0, mh);
			String property = environment.getProperty(eKey, mh == -1 ? null : idEntry.getValue().substring(mh + 1));
	 ...
	 * @return 替换结果
	 */
	default String replaceMethod(Object curObj, String val){
		Class<? extends Object> curObjClass = curObj.getClass();
		/* 动态替换方法返回值 */
		for (Entry<String, String> cv : MyUtil.getContainValue("&{", "}", val).entrySet()) {
			...
			try {
				...
				/* 执行结果 */
				property = declaredMethod.invoke(curObj, new Object[0]);
			} catch (Exception e) {throw new RuntimeException(String.format("当前类[%s]对应注解未找到执行方法[%s]，请检查注解参数值！", curObjClass.getName(), cv.getValue()));}
