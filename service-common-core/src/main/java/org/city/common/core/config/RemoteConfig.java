...
import org.city.common.api.in.scanner.Scanner;
...
public class RemoteConfig extends AppenderConfig implements GlobalConfig,Replace,FirstCharParse,MethodNameParse {
	...
	protected void init() {
		...
		Class<?> mainClass = ClassUtils.getUserClass(mainBean);
		String[] scanBasePackages = mainClass.getDeclaredAnnotation(SpringBootApplication.class).scanBasePackages();
		if (scanBasePackages.length == 0) {
			scanBasePackages = new String[] { mainClass.getPackage().getName() };
		}
