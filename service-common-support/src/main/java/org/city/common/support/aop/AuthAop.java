...
import org.city.common.api.adapter.impl.TimeAdapter;
...
public class AuthAop implements Replace,JSONParser,MethodNameParse {
	...
	private final RemoteAdapter idAdapter = new RemoteAdapter() {
		@Override
		public RemoteInfo select(List<RemoteInfo> remoteInfos, Object id) {
			List<RemoteInfo> matchs = remoteInfos.stream().filter(v -> getId(v.getRemoteClassDto()).equals(id)).collect(Collectors.toList());
			return timeAdapter.select(matchs, id);
		}
	};
	...
	public Object authFilterAround(ProceedingJoinPoint jp) {
		...
		RemoteMethodDto authMethod = (RemoteMethodDto) args[0]; //验证的原方法信息
		
		/* 调用原验证方法进行验证 - 返回NULL跳过验证 */
		AuthResultDto auth = authProvider.auth(authMethod, (boolean) args[1], (Object[]) args[2], (int) args[3], (String[]) args[4]);
		if (auth == null) {return null;}
		/* 当不成功时 - 且标记抛出异常 - 则抛出异常 */
		if (!auth.isSuccess() && (boolean) args[1]) {throw auth.getAuthNotPassException();}
	...
	@Before("authCut()")
	public void authBefore(JoinPoint jp) throws Exception {
	...
	/* 验证参数 */
	private AuthResultDto auth(Auth auth, Object[] datas, String[] names, Object curObj, Method method, boolean isThrow) throws Exception {
		/* 完善自定义values值 */
		...
		return PlugUtil.invoke(auth.id(), idAdapter, AuthProvider.class, new ClassProcess<AuthProvider, AuthResultDto>() {
			...
			public AuthResultDto invoke(AuthProvider process) {
				return process.auth(authScanner.getByMethod(method), isThrow, clones, auth.value(), values);
			}
		});
	}
}
