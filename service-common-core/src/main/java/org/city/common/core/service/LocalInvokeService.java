...
public class LocalInvokeService extends Configurator implements JSONParser,MethodNameParse,ThrowableMessage {
	...
	private final static Task TASK = SpringUtil.getBean(Task.class);
	...
	public void onOpen(Session session) {
		Assert.isTrue(REMOTE_CONFIG_DTO.isInvoke(), "服务器未开启远程调用功能，执行失败！");
		Object auth = session.getUserProperties().get(CommonConstant.AUTHORIZATION.toLowerCase());
		Assert.isTrue(MyUtil.sha256(REMOTE_CONFIG_DTO.getVerify()).equals(auth), "服务器Key认证失败，无法远程调用！");
	}
	...
	public void onMessage(byte[] body, Session session) {
		TASK.putTask(new Runnable() {
			...
			public void run() throws Throwable {
				try {
					HeaderUtil.set(remote.getHeaders()); RemoteAdapter.REMOTE_TRANSACTIONAL.set(remote.getTranId());
					Result result = localInvoke(remote.getBeanName(), remote.getRemoteMethod().getName(), remote.getArgs());
					...
					Result result = new Result().setResponse(Response.error(globalException, exception.getErrorMsg())).$setReturnType(GlobalExceptionDto.class);
					sendResult(session, JSON.toJSONBytes(result.setRequestId(remote.getRequestId())));
				} finally {RemoteAdapter.REMOTE_TRANSACTIONAL.remove(); HeaderUtil.remove();}
			}
			/* 发送响应结果 */
	...
	private Result localInvoke(String beanName, String methodName, Object[] args) {
		...
		return new Result().setResponse(Response.ok(returnValue)).$setReturnType(entry.getValue());
	}
