...
public class RedisRemoteSaveService implements RemoteSave,JSONParser,ApplicationRunner {
	...
	@Override
	public void init() {
		ONLINE.add(localIpPort.toString()); //自己服务上线 - 解决Controller未初始化完成无法调用自己的接口
		...
		redisMake.setValue(localIpPort.toString() + CommonConstant.REDIS_REMOTE_PREFIX_KEY, true, 1, TimeUnit.HOURS);
	}
	...
	@Override
	public void flush() {
		Set<String> curOnline = new HashSet<>(); //调用每个服务获取IP端口
		manyRequest(cia -> { //查询所有服务
			RemoteIpPortDto remoteIpPort = ((CommonInfoApi) cia.getBean()).getIpPort(); //远程服务IP端口
			if (cia.getRemoteIpPortDto().equals(remoteIpPort)) {curOnline.add(remoteIpPort.toString());}
			else {throw new ConnectException("远程服务非真实服务！");} //服务IP端口不一致则为脏数据
		});
		
		/* 如果不相同则说明服务有变化 */
		if (!JSON.toJSONString(curOnline).equals(JSON.toJSONString(ONLINE))) {
			ONLINE.clear(); ONLINE.addAll(curOnline);
		}
	...
	public List<RemoteInfo> get(String methodName) {
		...
		if (remoteCache == null) {
			remoteCache = new RemoteCache();
			setRedisRemote("-" + methodName, remoteCache.getRemoteInfos());
	...
	private void setRedisRemote(String scopeKey, List<RemoteInfo> remotes) {
		...
		for (Entry<String, Map<String, List<RemoteClassDto>>> scopeRemote : getScopeRemote(scopeKey).entrySet()) {
			for (Entry<String, List<RemoteClassDto>> entry : scopeRemote.getValue().entrySet()) {
				...
				/* 公共接口或线上服务接口才添加 */
				if (interfaceCls == CommonInfoApi.class || ONLINE.contains(entry.getKey())) {
