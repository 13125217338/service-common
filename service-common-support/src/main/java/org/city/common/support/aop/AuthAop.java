...
public class AuthAop implements Replace,JSONParser,MethodNameParse {
	...
	public Object authFilterAround(ProceedingJoinPoint jp) {
		...
		AuthMethod authMethod = (AuthMethod) args[0];
		
		/* 调用原验证方法进行验证 */
		AuthResultDto auth = authProvider.auth(authMethod, (Object[]) args[1], (String[]) args[2]);
		if (auth == null) { //当返回值为空且最后一个验证时不可跳过
			if (authMethod.getIsThrow() == null) {throw new AuthNotPassException("所有验证返回均为空，验证失败！");} else {return null;}
		}
		/* 当不成功时 - 只要最后一个验证或者标记抛出异常 - 则都抛出异常 */
		if (!auth.isSuccess() && (authMethod.getIsThrow() == null || authMethod.getIsThrow())) {throw auth.getAuthNotPassException();}
	...
	public void authBefore(JoinPoint jp) {
		...
		/* 根据类型执行验证 */
		switch (auths.type()) {
			case AND: for (int i = 0, j = auths.auths().length; i < j; i++) {
							Boolean isThrow = ((i + 1) == j) ? (result == null ? null : Boolean.TRUE) : Boolean.TRUE;
							result = auth(auths.auths()[i], datas, names, jp.getTarget(), method, isThrow);
					  }
			case OR: for (int i = 0, j = auths.auths().length; i < j; i++) {
							Boolean isThrow = ((i + 1) == j) ? (result == null ? null : Boolean.TRUE) : Boolean.FALSE;
							result = auth(auths.auths()[i], datas, names, jp.getTarget(), method, isThrow);
							/* or条件成功则直接返回 */
	...
	private AuthResultDto auth(Auth auth, Object[] datas, String[] names, Object curObj, Method method, Boolean isThrow) {
		...
		return PlugUtil.invoke(auth.id(), idAdapter, AuthProvider.class, new ClassProcess<AuthProvider, AuthResultDto>() {
			...
			public AuthResultDto invoke(AuthProvider process) {
				return process.auth(toAuthMethod(method, auth, null).setIsThrow(isThrow), clones, values);
			}
		});
	}
}
