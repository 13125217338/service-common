...
import org.city.common.api.open.RedisCacheFlushApi;
 ...
 */
@Component
public class RedisCacheMake implements MakeInvoke,RedisCacheFlushApi,JSONParser {
	@Autowired
	...
	public void invoke(Process process, int value, String[] values) throws Throwable {
		...
		/* 如果是刷新方式 - 删除Redis缓存 - 并刷新其他服务本地缓存 */
		boolean isFlush = values.length > 1 ? Boolean.TRUE.toString().equalsIgnoreCase(values[1]) : false;
		if (isFlush) {
			redisMake.delKey(values[0]);
			PlugUtil.getBeans(RedisCacheFlushApi.class).forEach(v -> v.flush(values[0]));
		}
	...
	private void setLocal(String key, Object data, int waitTime) {
		if (cacheSize > 0) { //本地缓存数量大于0才有效
			try {
				READ_WRITE_LOCK.writeLock().lock();
				while (LOCAL_CACHE.size() > cacheSize) { //如果本地缓存超过设置值 - 删除头数据，追加尾数据
	...
	private void verifyExpire(String key) {
		Entry<Object, Long> cache = null;
		try {
			READ_WRITE_LOCK.readLock().lock();
			cache = LOCAL_CACHE.get(key);
		} finally {
			READ_WRITE_LOCK.readLock().unlock();
		}
		
		// 如果缓存存在且已过期则删除本地缓存
		if (cache != null && System.currentTimeMillis() > cache.getValue()) {
			flush(key);
		}
	}
	/* 获取记录时间对应本地缓存 */
